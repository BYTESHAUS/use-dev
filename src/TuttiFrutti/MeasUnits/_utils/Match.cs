...
public static class Match
{
    ...
    public static bool CouldLoose<U>(string raw, out U? unit) where U : Enum {
    ...
    public static (bool success, U? unit) Loose<U>(string raw) where U : Enum {
        ...
        if (!match.success && FactoredAttribute.From(typeof(U)).Any())
            match.success = CouldMetricMatch<U>(raw, out match.unit);
    ...
    private static bool CouldMetricMatch<U>(string raw, out U? match) where U : Enum {
        match = default;
